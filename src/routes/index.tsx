      ...
      {step === 1 && (<>
        {competitors.map((c, i) => (
          <div key={i} className="competitor-card">
            ...
            <input type="text" value={c.name} onChange={(e) => updateCompetitor(i, { name: e.target.value })} placeholder={i === 0 ? "e.g. Figma" : i === 1 ? "e.g. Notion" : ""} />
            <label>Seed URLs</label>
            <textarea value={c.urls} onChange={(e) => updateCompetitor(i, { urls: e.target.value })} placeholder={i < 2 ? "https://help.example.com/permissions" : ""} />
          </div>
        ))}
